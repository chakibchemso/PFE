    ...
    MODE: ChannelCount<IC, MODE>,
{
    ...
    /// ADC resolution.
    pub async fn read_fifo(&mut self, output_data: &mut [u32]) -> Result<u8, Error<E>> {
        let mode_channels = usize::from(MODE::CHANNEL_COUNT);

        if output_data.len() < mode_channels {
            return Ok(0);
        }
        let samples = self.get_available_sample_count().await?;
        ...
        if sample_count != 0 {
            self.read_samples(sample_count, output_data).await?;
        }
    ...
    I2C: i2c::I2c<Error = E>,
{
    ...
    pub async fn get_available_sample_count(&mut self) -> Result<u8, Error<E>> {
        let mut data = [0; 3];
        self.read_data(Register::FIFO_WR_PTR, &mut data).await?;
    ...
    pub async fn get_overflow_sample_count(&mut self) -> Result<u8, Error<E>> {
        let v = self.read_register(Register::OVF_COUNTER).await?;
        Ok(v & 0x1F)
    }
    ...
    pub async fn read_interrupt_status(&mut self) -> Result<InterruptStatus, Error<E>> {
        let mut data = [0; 2];
        self.read_data(Register::INT_STATUS, &mut data).await?;
    ...
    /// Get revision ID
    pub async fn get_revision_id(&mut self) -> Result<u8, Error<E>> {
        self.read_register(Register::REV_ID).await
    }
